const City = ["Toshkent", "Namangan", "Andijon", "Farg'ona",  "Buxoro", "Qashqadaryo", "Surxandaryo",  "Samarqand", "Qoraqalpog'iston"  ];
const Position = ["React","Php developer", "Nodejs", "Vue js developer", "Go", "Python"];
...
["Select a position type", ...TypePosition].forEach((typeposition) => {
  filterPosition.innerHTML += `<option value="${typeposition}">${typeposition}</option>`;
});

["Select an address", ...City].forEach((city) => {
...
filterPosition.addEventListener("change", function () {
  if (this.value == "Select a position type") {
    getPupils();
  } else {
...
filterCity.addEventListener("change", function () {
  if (this.value == "Select an address") {
    getPupils();
  } else {
